...
class BaseClient {
    constructor(clientId, clientSecret, clientVersion, env, shouldPublishEvents) {
        ...
        this.requestViaAuthRefreshPci = (method, url, responseType, headers, data, pathParams) => __awaiter(this, void 0, void 0, function* () {
            return this.executeWithRetry(this._pciHttpCommand, method, url, responseType, headers, data, pathParams);
        });
        this.executeWithRetry = (httpCommand, method, url, responseType, headers, data, pathParams) => __awaiter(this, void 0, void 0, function* () {
            const httpHeaders = yield this.addAuthHeader(headers);
            try {
                const response = yield httpCommand.request(url, method, httpHeaders, data, pathParams);
                return (0, class_transformer_1.plainToClass)(responseType, response);
            }
            catch (error) {
                if (error instanceof Exceptions_1.UnauthorizedAccess) {
                    yield this.tokenService.forceRefreshToken();
                    const retryHeaders = yield this.addAuthHeader(headers);
                    const retryResponse = yield httpCommand.request(url, method, retryHeaders, data, pathParams);
                    return (0, class_transformer_1.plainToClass)(responseType, retryResponse);
                }
                throw error;
            }
        });
        ...
        this._pciHttpCommand = new HttpCommand_1.HttpCommand(EnvConfig_1.EnvConfig.getBaseUrls(env).pciPgHostUrl, this._httpClient);
